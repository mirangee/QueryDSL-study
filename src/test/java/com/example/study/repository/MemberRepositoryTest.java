...
//                .where(원하는 조건)
    ...
    */
    ...
    void subQueryTest() {
        ...
        List<Member> result = factory.selectFrom(member)
                .where(member.age.eq(
                        ...
                        JPAExpressions // 서브쿼리를 사용할 수 있게 도와주는 클래스(WHERE절, SELECT절 가능 / From절(인라인뷰) 불가
    ...
    void subQueryGoeTest() {
        ...
        List<Member> result = factory.selectFrom(member)
                .where(member.age.goe(
                        JPAExpressions
                                // JPAExpressions는 from 절 제외한 select, where 절에서 사용 가능
                                // JPQL도 마찬가지로 from절 서브쿼리 사용 불가
                                // -> Native SQL 사용하든지 MyBatis or JDBC Template 이용, 따로 따로 두 번 조회 사용
    ...
    @Test
    @DisplayName("동적 SQL TEST")
    void dynamicQueryTest() {
        // given
        // null은 참조할 값이 없다는 뜻, 고로 객체 타입만 null값을 가질 수 있다.
        String name = "";
        Integer age = null;
